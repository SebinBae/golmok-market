...
- 비밀번호 재설정은 **`LOCAL` + `emailVerified = true`인 credential에만** 허용한다.
  ⚠️ **M1에서는 `emailVerified` 조건을 적용하지 않는다** — 이메일 인증 절차가 없어
  조건을 걸면 아무도 재설정할 수 없다. 소셜 로그인 도입 시 켠다 (`decisions/0009`)
...
- **해싱은 SHA-256이다** (`token_hash` → `varchar(64)`). 인증번호에 BCrypt를 쓴 것과 정반대인데,
  토큰이 랜덤 32바이트라 전수 대입이 불가능해 느린 해시가 지켜줄 것이 없다.
  결정적 해시여야 `token_hash`로 바로 조회할 수 있다는 점도 있다
- 검증 조건: 해시 일치 + `expiresAt > now` + `usedAt IS NULL`
- 새 토큰을 발급하면 **기존 미사용 토큰을 삭제해 무효화한다.** 살아있는 링크가 여럿이면
  유출된 옛 링크로도 비밀번호를 바꿀 수 있다. 사용된 토큰은 이력으로 남긴다
...
되돌려 무차별 대입 방어가 무력해진다. 실측 확인: 방어가 없으면 5번 틀려도 `attempt_count`가 0이다.

`noRollbackFor`를 서비스에 거는 방법을 먼저 썼으나 **호출자가 감싸면 다시 무너졌다.**
`FindEmailService`가 `verify()`를 자기 트랜잭션 안에서 부르자 바깥 롤백 규칙이 이겼다.
호출자마다 반복해 걸어야 하고, 한 곳만 빠뜨려도 아무 신호가 없다.

그래서 `VerificationAttemptRecorder`가 `REQUIRES_NEW`로 **별도 트랜잭션에서 커밋**한다.
호출자가 무엇이든 보장된다.

이 때문에 **시도 횟수를 확인하는 테스트는 `@Transactional`을 걸면 안 된다.**
아직 커밋되지 않은 행을 새 트랜잭션이 볼 수 없어 증가가 조용히 사라진다.
...
- **아이디 찾기 결과는 반드시 마스킹**한다: `hy****@gmail.com`
  마스킹 규칙은 **앞 2자 + 고정 `****` + 도메인 원본**이다 (`EmailMasker`).
  별표를 가려진 길이만큼 찍으면 로컬파트 길이가 새어나가므로 개수를 고정한다.
- 인증번호를 통과했다는 것은 그 번호의 소유자라는 뜻이므로,
  가입 내역이 없으면 **없다고 그대로 알린다.** 열거 공격은 인증번호 단계에서 이미 막힌다
- M1에서 SMS 발송은 콘솔 출력
